...
// mockShareDeleter is a test double for the shareDeleter interface.
type mockShareDeleter struct {
	err              error
	called           bool
	calledTargetType domain.ShareTargetType
	calledTargetID   string
}

func (m *mockShareDeleter) DeleteByTarget(_ context.Context, targetType domain.ShareTargetType, targetID string) error {
	m.called = true
	m.calledTargetType = targetType
...
func TestItemServiceAssignLocationShouldReturnErrorWhenContextIsCancelled(t *testing.T) {
	svc := NewItemService(&mockItemRepo{}, &mockMediaProvider{}, &mockLocationRepo{}, NewCategoryService(), &mockShareAccessChecker{}, &mockShareDeleter{})
...
func TestItemServiceAssignLocationShouldReturnErrorWhenItemStoreGetFails(t *testing.T) {
	...
	svc := NewItemService(repo, &mockMediaProvider{}, &mockLocationRepo{}, NewCategoryService(), &mockShareAccessChecker{}, &mockShareDeleter{})
...
func TestItemServiceAssignLocationShouldReturnErrNotFoundWhenLocationDoesNotExist(t *testing.T) {
	...
	svc := NewItemService(itemRepo, &mockMediaProvider{}, &mockLocationRepo{}, NewCategoryService(), &mockShareAccessChecker{}, &mockShareDeleter{})
...
func TestItemServiceAssignLocationShouldReturnErrForbiddenWhenLocationBelongsToDifferentOwner(t *testing.T) {
	...
	svc := NewItemService(itemRepo, &mockMediaProvider{}, locRepo, NewCategoryService(), &mockShareAccessChecker{}, &mockShareDeleter{})
...
func TestItemServiceCreateShouldCreateItemWithCallerAsOwner(t *testing.T) {
	...
	svc := NewItemService(repo, &mockMediaProvider{}, &mockLocationRepo{}, catSvc, &mockShareAccessChecker{}, &mockShareDeleter{})
...
func TestItemServiceGetByIDShouldReturnErrorWhenShareCheckerFails(t *testing.T) {
	...
	repo := &mockItemRepo{items: []domain.Item{item}}
	svc := NewItemService(repo, &mockMediaProvider{}, &mockLocationRepo{}, NewCategoryService(), shareChecker, &mockShareDeleter{})
...
func TestItemServiceUpdateShouldSetLocationIDWhenNullableValInInput(t *testing.T) {
	...
	repo := &mockItemRepo{items: []domain.Item{item}}
	locRepo := &mockLocationRepo{locations: []domain.Location{loc}}
	svc := NewItemService(repo, &mockMediaProvider{}, locRepo, NewCategoryService(), &mockShareAccessChecker{}, &mockShareDeleter{})
...
func TestItemServiceUploadPhotoShouldReturnErrorWhenUploadFails(t *testing.T) {
	...
	svc := NewItemService(repo, media, &mockLocationRepo{}, NewCategoryService(), &mockShareAccessChecker{}, &mockShareDeleter{})
...
func TestItemServiceDeleteShouldSucceedWhenShareDeleteFails(t *testing.T) {
	var item domain.Item
	item.ID = "item-1"
	item.OwnerID = "owner-1"
	...
	svc := NewItemService(repo, &mockMediaProvider{}, &mockLocationRepo{}, NewCategoryService(), &mockShareAccessChecker{}, sd)

	err := svc.Delete(t.Context(), "owner-1", "item-1")
	require.NoError(t, err)
}

func TestItemServiceDeleteShouldCallShareDeleteByTargetWithItemTargetWhenSuccessful(t *testing.T) {
	var item domain.Item
	item.ID = "item-1"
	item.OwnerID = "owner-1"
	...
	require.Equal(t, domain.ShareTargetItem, sd.calledTargetType)
	require.Equal(t, "item-1", sd.calledTargetID)
}
