...
type OutfitLogRepository interface {
	// Get retrieves a single outfit log by ID, including linked wear log IDs.
	// Returns domain.ErrNotFound if no outfit log with that ID exists.
	Get(ctx context.Context, id string) (domain.OutfitLog, error)

	// Save upserts the outfit log row only (not wear log links).
	...
	// ListByOwnerDateRange returns logs where worn_on is between from and to
	// (inclusive), ordered by worn_on ascending. Each log includes its linked
	// wear log IDs.
