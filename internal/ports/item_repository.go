...
type ItemRepository interface {
	...
	// Save creates or updates the item. It does NOT touch photo entries; use
	...
	// Each returned item includes its photo keys. Implementations must load
	// all photos in a single batched call rather than one per item.
	...
	// SavePhoto saves a photo entry linking photoID and mediaKey to itemID
	// at the given position.
	SavePhoto(ctx context.Context, itemID, photoID, mediaKey string, position int) error

	// DeletePhoto removes the photo entry identified by itemID + mediaKey.
