...
func TestRegisterHandlerShouldReturn409WhenEmailAlreadyTaken(t *testing.T) {
	...
	w := postRegister(t, h, `{"email":"alice@example.com","password":"secret"}`)

	require.Equal(t, http.StatusConflict, w.Code)
	var body map[string]string
	require.NoError(t, json.NewDecoder(w.Body).Decode(&body))
	require.Equal(t, "email already taken", body["error"])
}
...
func TestLoginHandlerShouldReturn401WhenCredentialsAreInvalid(t *testing.T) {
	...
	w := postLogin(t, h, `{"email":"alice@example.com","password":"wrong"}`)
...
func TestLoginHandlerShouldReturn500WhenServiceFails(t *testing.T) {
	...
	w := postLogin(t, h, `{"email":"alice@example.com","password":"secret"}`)
