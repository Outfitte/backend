...
- **Outfits** — build named outfits from multiple items, attach photos, and log
  outfit wear events on a calendar.
- **Outfit calendar** — list and delete outfit-wear log entries; logging an
  outfit automatically creates a wear-log entry for each item in it.
...
- **Docker Compose self-hosting** — run a complete Outfitte instance (API +
  SQLite + local media storage) with a single `docker compose up`.
